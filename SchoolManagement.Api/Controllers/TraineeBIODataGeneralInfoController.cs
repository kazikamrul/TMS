...
public class TraineeBioDataGeneralInfoController : ControllerBase
{
    ...
    [HttpGet]
    [Route("get-traineeListForCreateUser")]
    public async Task<ActionResult<List<TraineeBioDataGeneralInfoDto>>> GetTraineeListForCreateUser([FromQuery] QueryParams queryParams)
    {
        var TraineeBioDataGeneralInfos = await _mediator.Send(new GetTraineeListForCreateUserRequest { QueryParams = queryParams });
        return Ok(TraineeBioDataGeneralInfos);
    }
